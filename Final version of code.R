##### FERDIG VERSJON. LIMER KUN INN DET SOM ER FERDIG I DENNE 
...
#First part is to install packages. If you run jupyter, do not comment them out. 
...
bf2017['D_PBD_decr'].fillna(0, inplace=True)

# Calculate cumulative sum by state
bf2017['PBD_decr'] = bf2017.groupby('state')['D_PBD_decr'].cumsum()

# Summary statistics of PBD_decr
summary_PBD_decr = bf2017['PBD_decr'].describe()
print(summary_PBD_decr)

# Estimate distributed-lag model
exog_vars = ['lead(PBD_decr, 2)', 'lead(PBD_decr, 1)', 'PBD_decr', 
             'lag(PBD_decr, 1)', 'lag(PBD_decr, 2)', 'lag(PBD_decr, 3)', 'lag(PBD_decr, 4)',
             'frac_total_ui', 'yearmonth']
model_formula = 'np.log(GJSI) ~ ' + ' + '.join(exog_vars)
model_data = bf2017[bf2017['year'] >= 2012]
estim_decr_dl_fe = PanelOLS.from_formula(model_formula, data=model_data, effect='individual', 
                                         entity_effects=True)
summary_estim_decr_dl_fe = estim_decr_dl_fe.fit()
print(summary_estim_decr_dl_fe)

# Gamma coefficients
gamma_decr_dl_fe = summary_estim_decr_dl_fe.params[:7]

# Store variance-covariance matrix
vcov_decr_dl_fe = summary_estim_decr_dl_fe.cov[:7,:7]

# Beta coefficients and standard errors
beta_decr_dl_fe = pd.DataFrame({
  'month_to_reform': [-3, -2, -1, 0, 1, 2, 3, 4],
  'coef': [-np.cumsum(gamma_decr_dl_fe[:2])[::-1], 0, np.cumsum(gamma_decr_dl_fe[2:])],
  'se': [-np.sqrt(np.cumsum(np.diag(vcov_decr_dl_fe)[:2]))[::-1], 0, np.sqrt(np.cumsum(np.diag(vcov_decr_dl_fe)[2:]))]
})

# Plot beta coefficients
plt.figure()
plt.errorbar(x=beta_decr_dl_fe['month_to_reform'], y=beta_decr_dl_fe['coef'], 
             yerr=1.96*beta_decr_dl_fe['se'], color='darkblue', fmt='o')
plt.axhline(y=0, color='black')
plt.axvline(x=-0.5, linestyle='dashed')
plt.xticks(np.arange(-3, 5))
plt.xlabel("Months relative to reform \n Observations: {}, states: {}, periods: {} ({} - {})."
           .format(estim_decr_dl_fe.nobs,
                   estim_decr_dl_fe.model.dataframe['state'].nunique(),
                   estim_decr_dl_fe.model.dataframe['yearmonth'].nunique(),
                   estim_decr_dl_fe.model.dataframe['yearmonth'].min(),
                   estim_decr_dl_fe.model.dataframe['yearmonth'].max()))
plt.ylabel("Effect on log search intensity")
plt.ylim(-0.045, 0.04)
plt.yticks(np.arange(-0.04, 0.041, 0.02))
plt.grid(True)
plt.show()
...
estim_incr_es_fe = PanelOLS.from_formula(model_formula, data=model_data, effect='individual', 
                                         entity_effects=True)
